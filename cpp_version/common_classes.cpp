...
public:
    ...
    string task_id;
    Algo(vector<Item> items, Rect material, string task_id=""){
        this->items = items;
        this->material = material;
...
// PYBIND11_MODULE(common_classes, m) {
//     py::class_<Algo>(m, "Algo")
//         .def(py::init<const std::vector<Item>&, const Rect&, const std::string&>(), py::arg("items"), py::arg("material"), py::arg("task_id")="")
//         .def_readwrite("items", &Algo::items)
//         .def_readwrite("material", &Algo::material)
//         .def_readwrite("solution", &Algo::solution)
//         .def_readwrite("task_id", &Algo::task_id);
// }   